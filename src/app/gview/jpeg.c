...
struct DLL_STR_PIC_ENV { int work[16384]; };
...
int info_JPEG(struct DLL_STR_PIC_ENV *env, int *info, int size, UCHAR *fp);
int decode0_JPEG(struct DLL_STR_PIC_ENV *env, int size, UCHAR *fp, int b_type, UCHAR *buf, int skip);
...
int info_JPEG(struct DLL_STR_PIC_ENV *env,int *info, int size, UCHAR *fp0)
{
...
int decode0_JPEG(struct DLL_STR_PIC_ENV *env,int size, UCHAR *fp0, int b_type, UCHAR *buf, int skip)
{
	JPEG *jpeg = (JPEG *) (((int *) env) + 128);
	jpeg->fp = fp0;
	jpeg->fp1 = fp0 + size;
...
#define INIT_Z_TABLE(i, b0, b1, b2, b3)	*(int *) &zigzag_table[i] = b0 | b1 << 8 | b2 << 16 | b3 << 24
...
void jpeg_decode(JPEG *jpeg, UCHAR *rgb, int b_type)
{
	...
	INIT_Z_TABLE( 0,  0,  1,  8, 16); INIT_Z_TABLE( 4,  9,  2,  3, 10);
	INIT_Z_TABLE( 8, 17, 24, 32, 25); INIT_Z_TABLE(12, 18, 11,  4,  5);
	INIT_Z_TABLE(16, 12, 19, 26, 33); INIT_Z_TABLE(20, 40, 48, 41, 34);
	INIT_Z_TABLE(24, 27, 20, 13,  6); INIT_Z_TABLE(28,  7, 14, 21, 28);
	INIT_Z_TABLE(32, 35, 42, 49, 56); INIT_Z_TABLE(36, 57, 50, 43, 36);
	INIT_Z_TABLE(40, 29, 22, 15, 23); INIT_Z_TABLE(44, 30, 37, 44, 51);
	INIT_Z_TABLE(48, 58, 59, 52, 45); INIT_Z_TABLE(52, 38, 31, 39, 46);
	INIT_Z_TABLE(56, 53, 60, 61, 54); INIT_Z_TABLE(60, 47, 55, 62, 63);
