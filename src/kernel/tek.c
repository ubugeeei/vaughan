...
int tek_getsize(unsigned char *p) {
    // clang-format off
    static char header[15] = {0xff, 0xff, 0xff, 0x01, 0x00, 0x00, 0x00, 0x4f, 0x53, 0x41, 0x53, 0x4b, 0x43, 0x4d, 0x50};
    // clang-format on
    int size = -1;
    if (memcmp(p + 1, header, 15) == 0 &&
        (*p == 0x83 || *p == 0x85 || *p == 0x89)) {
        p += 16;
...
static int tek_lzrestore_tek5(int srcsiz, UCHAR *src, int outsiz, UCHAR *outbuf) {
    ...
    work = (int *)memman_alloc_4k((struct MemoryManagement *)MEMMAN_ADDR, wrksiz);
    if (work == NULL) return -1;
    flags = tek_decmain5(work, src, outsiz, outbuf, lc, pb, lp, flags);
    memman_free_4k((struct MemoryManagement *)MEMMAN_ADDR, (int)work, wrksiz);
    return flags;
}
...
static int tek_rdget1(struct tek_STR_RNGDEC *rd, tek_TPRB *prob0, int n, int j,
    ...
    do {
        ...
        if (bm->lt > 0) {
            if (--bm->lt == 0) {
                if (tek_rdget1(rd, &rd->probs.fchglt, 0x71, 0, &rd->bm[3]) ==
                    0) {
                err:
...
static int tek_getlen5(struct tek_STR_RNGDEC *rd, int m, int s_pos, int stk) {
    ...
    // clang-format on
        ...
        /* high */
        i = tek_rdget1(rd, rd->probs.lenhigh[m], 0x78, 1, rd->ptbm[6]) -
            (256 + 256 - 8);
        if (i > 0) {
            if (i < 6 && stk == 0)
                // clang-format off
...
static int tek_decmain5(int *work, UCHAR *src, int osiz, UCHAR *q, int lc, int pb, int lp, int flags) {
    ...
    if ((tek_rdget1(rd, &prb->pb[0].st[0].mch, 0x71, 0, rd->ptbm[0]) ^ stk) ==
        0)
        goto err;
    *q++ = tek_rdget1(rd, prb->lit, lit0cntmsk, 1, &rd->bm[24]) & 0xff;
    ...
    while (pos < osiz) {
        s_pos = pos & m_pos;
        // clang-format off
        if (tek_rdget1(rd, &prb->pb[s_pos].st[s].mch, 0x71, 0, rd->ptbm[s > 0]) ^
            stk) {
            ...
            pmch |= 1;
            if (tek_rdget1(rd, &prb->st[s].rep, 0x71, 0, rd->ptbm[13]) ^
                stk) { /* len/dis */
                rep[3] = rep[2];
                rep[2] = rep[1];
                rep[1] = rep[0];
                ...
                if (j >= 4) {
                    ...
                    if (j < 14) /* k < 6 */
                        rep[0] |= tek_revbit(
                            tek_rdget1(rd, &prb->spdis[j & 1][(1 << k) - 2], k | 0x70, 1, rd->ptbm[10 + (k >= 4)]), k);
                    else {
                        if (stk == 0) {
                ...
                if (tek_rdget1(rd, &prb->st[s].repg0, 0x71, 0, rd->ptbm[13]) ^
                    stk) { /* rep0 */
                    i |= -1;
                    ...
                    if (tek_rdget1(rd, &prb->st[s].repg1, 0x71, 0, rd->ptbm[13]) ^ stk) /* rep1 */
                        ...
                        // clang-format off
                        if (tek_rdget1(rd, &prb->st[s].repg2, 0x71, 0, rd->ptbm[13]) ^ stk) /* rep2 */ i = rep[2];
                        // clang-format on
                        else {
                            if (stk == 0) {
                                // clang-format off
